...
if (@ARGV != 3) {
  ...
  print STDERR "e.g. $0 /export/corpora/SRE18/SRE18_dev_SPH/\n";
  exit(1);
}
...
$out_dir_unlabeled = "$out_dir/sre18_unlabeled_SP$speedup";
...
if (system("find $db_base/data_SP$speedup/unlabeled/major/ -name '*.wav' > $tmp_dir_unlabeled/wav.list") != 0) {
  die "Error getting list of sph files";
}

open(WAVLIST, "<$tmp_dir_unlabeled/wav.list") or die "cannot open wav list";
