...
# For checking up, ensure fix_data_dir.sh: kept all 1757 utterances (instead of 1759)
...
# Handle enroll
$out_dir_enroll = "$out_dir/sre20_eval_enroll_SU$speedup";   # Create data/sre20_eval_enroll_su08 folder
...
open(META, "<$db_base/docs/2020_cts_challenge_enrollment.tsv") or die "cannot open wav list - $db_base/docs/2020_cts_challenge_enrollment.tsv , $out_dir_enroll ";
%utt2fixedutt = ();
while (<META>) {
  $line = $_;
  @toks = split(" ", $line);
...
if (system("find $db_base/data_$speedup/enrollment/ -name '*.wav' > $tmp_dir_enroll/wav.list") != 0) {
...
# Handle test
$out_dir_test= "$out_dir/sre20_eval_test_SU$speedup";
...
if (system("find $db_base/data_$speedup/test/ -name '*.wav' > $tmp_dir_test/wav.list") != 0) {
  die "Error getting list of wav files";
}

open(KEY, "<$db_base/docs/2020_cts_challenge_trials.tsv") || die "Could not open trials file $db_base/docs/2020_cts_challenge_trials.tsv.  It might be located somewhere else in your distribution.";
