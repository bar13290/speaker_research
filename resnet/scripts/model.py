...
class NeuralSpeakerModel(nn.Module):
    ...
    def __init__(self, model, input_dim, output_dim, D, hidden_dim=128, distance_type='norm', network_type='att', pooling='mean', asoftmax=False, m=2):
        ...
        if model == 'resnet34':
            self.res = resnet34().to(torch.device("cuda:0"))
            _feature_dim = 128
            self.res = torch.nn.DataParallel(self.res, device_ids=[0, 1]) #parallelize for multiple GPU's
    ...
    def forward(self, x):
        ...
        if self.asoftmax == 'True':
            ...
            ww = w.renorm(2,1,1e-5).mul(1e5)      # normalize each weight column to 1 ||w||=1
            xlen = x.pow(2).sum(1).pow(0.5) # size=B -> norm2 of x (||x||)
            wlen = ww.pow(2).sum(0).pow(0.5) # size=Classnum
            cos_theta = x.mm(ww) # size=(B,Classnum)

            cos_theta = cos_theta / xlen.view(-1,1) / wlen.view(1,-1)
            cos_theta = cos_theta.clamp(-1,1)
            cos_m_theta = self.mlambda[self.m](cos_theta)   # coisine multi-angle formula
...
class AngleLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        index = index.detach().to(dtype=torch.bool)
