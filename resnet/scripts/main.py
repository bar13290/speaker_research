...
if __name__ == "__main__":
    ...
    print('use cuda is {},{}' .format(use_cuda,torch.cuda.is_available()) )
    ...
    for epoch in range(start_epoch, start_epoch+args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            data, target = data.to(device), target.to(device).view((-1,))#, non_blocking=True
            # data, target = data.to(device), target.to(device).view((-1,))#, non_blocking=True
            optimizer.zero_grad()
            output = model(data)
        ...
        total_loss /= batch_idx+1         # avarage loss per Batch
        loss_vec[0].append(total_loss)
        print('Total Loss = {} Time = {:.2f} minutes'.format(total_loss, (time()-s_time)/60) )
        model.eval()
        test_loss = 0
        correct = 0
        with torch.no_grad():
        ...
        test_loss /= len(val_loader.dataset)    # avarage loss per val example
        loss_vec[1].append(test_loss)
        test_loss_vec = [512*x for x in loss_vec[1]]
        ...
        if 100. * correct / len(val_loader.dataset) > best:
            ...
            best_epoch = epoch
        plt.figure()
        plt.plot(np.arange(1, epoch + 1), loss_vec[0], np.arange(1, epoch + 1), test_loss_vec)
        plt.grid(True)
        plt.legend(['Training Loss', 'Dev Loss'])
        plt.savefig(args.log_dir + 'loss_vs_epochs.png')
    #print(loss_vec)
    #plt.figure()
    #plt.plot(np.arange(start_epoch + args.epochs - 1), loss_vec[0], np.arange(start_epoch + args.epochs - 1), loss_vec[1])
    #plt.grid(True)
