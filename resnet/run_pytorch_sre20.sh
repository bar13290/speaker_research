...
sre20_trials_asnorm_enroll=trials_asnorm/asnorm_trials_sre20_enroll
sre20_trials_asnorm_test=trials_asnorm/asnorm_trials_sre20_test
...
############################ Scoring of SRE20 trials for asnorm #################################

if [ $stage -eq 531 ]; then


  plda=$backend_log/sre18_19_combined_no_sil_plda_voxswbdsre1M_lda_swbdsre_adapt
  mean=$backend_log/sre18_19_combined_no_sil_mean.vec
  lda=$backend_log/swbd_sre_combined_no_sil_transform.mat


  xvectors_cohort=$ivs/embedding_sre20_eval_enroll_no_sil.ark
  xvectors_dev_enroll=exp/xvectors_sre18_dev_enroll_Fv_skip_8k
  xvectors_dev_test=exp/xvectors_sre18_dev_test_Fv_skip_8k
  xvectors_eval_enroll=$ivs/embedding_sre20_eval_enroll_no_sil.ark
  xvectors_eval_test=$ivs/embedding_sre20_eval_test_no_sil.ark

echo " starting as-norm"
  as_norm.sh $mean $lda $plda $xvectors_cohort $xvectors_dev_enroll $xvectors_dev_test $xvectors_eval_enroll $xvectors_eval_test asnorm_scores/VR1
  echo "finish"
  wait
  exit 1
fi
