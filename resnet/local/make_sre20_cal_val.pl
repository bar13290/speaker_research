...
# Apache 2.0
# 2 enrollment files are exist on the data directory, but not exist in the docs:
#apgicaqc_sre20, apgkmibb_sre20
# Error will appear while running this script: Its ok, disregard it.
# Use of uninitialized value $utt in concatenation (.) or string at local/make_sre20_eval.pl line 57, <WAVLIST> line 1577
...
# Handle enroll
$out_dir_enroll = "$out_dir/sre20_enrollment_val";
...
open(META, "$db_base/enrollment_trials.tsv") or die "cannot open wav list - speaker_storage/storage_5/Calibration-LOO/2020_cts_challenge_enrollment.tsv , $out_dir_enroll ";
%utt2fixedutt = ();
...
if (system("find $db_base/enrollment_val/ -name '*.sph' > $tmp_dir_enroll/sph.list") != 0) {
...
# Handle test
$out_dir_test= "$out_dir/sre20_test_val";
...
if (system("find $db_base/test_val/ -name '*.sph' > $tmp_dir_test/sph.list") != 0) {
  die "Error getting list of sph files";
}

open(KEY, "<$db_base/test_trials.tsv") || die "Could not open trials file $db_base/docs/2020_cts_challenge_trials.tsv.  It might be located somewhere else in your distribution.";
#open(SEG_KEY, "</speaker_storage/storage_5/Calibration-LOO/validation.csv") || die "Could not open trials file $db_base/docs/sre18_eval_segment_key_SPH.tsv.  It might be located somewhere else in your distribution.";
